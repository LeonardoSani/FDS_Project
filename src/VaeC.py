...
def loss_function(x_hat, x, mu, logvar, beta=1):
    """
    Computes the VAE loss using Binary Cross Entropy.
    
    Important: 'x' (the target) must be in range [0, 1].
    """
    # reduction='sum' sums the loss over all pixels and batch elements
...
def train_vae(vae_model, X_train, X_val, epochs=50, batch_size=32, lr=1e-3, weight_decay=1e-4, beta=1, device=None):
    """
